...
function translate(lang, key, params = {}) {
	...
	for (const [name$1, value] of Object.entries(params)) text = text.split(`{${name$1}}`).join(String(value));
	return text;
}
...
const CANDIDATE_PREVIEW_CHARS = 80;
/**
...
*/
const DEFAULT_SNAPSHOT_ROOT = join(resolveDshHome(), SNAPSHOT_DIR_NAME);
/** Environment variable overriding the store root (tests, exotic homes). */
const SNAPSHOT_ROOT_ENV = "DSH_REWIND_SNAPSHOT_DIR";
...
async function isLinkPath(path) {
	try {
		...
		return stat$1.isSymbolicLink() || stat$1.nlink > 1;
	} catch {
		return false;
	}
}
...
async function dirSizeAndLastActive(dir) {
	...
	const visit = async (current) => {
		...
		for (const name$1 of names) {
			...
			await visit(join(current, name$1));
		}
	};
	await visit(dir);
	return {
...
var SnapshotStore = class SnapshotStore {
	...
	*/
	async listJournals(sessionId) {
		...
		const corrupt = [];
		for (const name$1 of names) {
			if (!name$1.startsWith(SnapshotStore.JOURNAL_PREFIX) || !name$1.endsWith(".json")) continue;
			try {
				const parsed = JSON.parse(await readFile(join(sessionDir, name$1), "utf8"));
	...
	*/
	async pruneTerminalJournals(sessionDir, names) {
		for (const name$1 of names) {
			if (!name$1.startsWith(SnapshotStore.JOURNAL_PREFIX) || !name$1.endsWith(".json")) continue;
			const file = join(sessionDir, name$1);
			try {
				const parsed = JSON.parse(await readFile(file, "utf8"));
	...
	*/
	async pruneStale(opts) {
		...
		for (const name$1 of names) {
			if (name$1.startsWith(".")) continue;
			const full = join(this.root, name$1);
			let st;
			try {
				st = await lstat(full);
			} catch {
			...
			scanned++;
			if (keepActiveId !== void 0 && safeSessionId(keepActiveId) === name$1) {
				skippedActive++;
	...
	*/
	async sessionStats(sessionId) {
		...
		for (const name$1 of names) {
			if (name$1.startsWith(".")) continue;
			const full = join(sessionDir, name$1);
			let st;
			try {
				st = await lstat(full);
			} catch {
			...
			if (st.isDirectory()) {
				if (!Number.isSafeInteger(Number(name$1))) continue;
				anchorGroups++;
				let files;
				try {
			...
			} else if (name$1.startsWith(SnapshotStore.JOURNAL_PREFIX) && name$1.endsWith(".json")) {
				journals++;
				bytes += st.size;
			}
		}
...
function parseCleanupCommand(rawInput) {
	...
	switch (parts[0]) {
		...
		case "run": {
			let apply$1 = false;
			let current = false;
			for (const rawFlag of parts.slice(1)) if (rawFlag === "--apply") apply$1 = true;
...
async function handleRewind(ctx, store, fs, invocation, inflight) {
	...
	if (parts[0] === "preview") {
		...
		let plan;
		try {
			plan = resolveOrError(eventsOf(session), session.surface.nodes, target$1);
		} catch (error) {
			return rewindErrorResult(error);
		}
	...
	if (parts[0] === "__restore") {
		...
		return executeRewind(ctx, store, fs, invocation, target$1, "both", inflight, false);
	}
...
async function handleSnapshotCleanup(store, invocation, dshHome, trackedBySession) {
	...
	switch (parsed.action) {
		...
		case "run": {
			const apply$1 = parsed.apply;
			if (parsed.target === "current") return handleClearCurrent(store, invocation, apply$1, trackedBySession);
			const loaded = await readCleanupPolicy();
			if (!loaded.ok) return {
				kind: "error",
				text: t("cleanup.cfgInvalid", { detail: loaded.error })
...
*/
async function handleClearCurrent(store, invocation, apply$1, trackedBySession) {
	const { agent } = invocation;
	const sessionId = agent.session.id;
	if (apply$1) {
		if (agent.status !== "idle") {
	...
	try {
		const report = await store.clearSession(sessionId, { dryRun: !apply$1 });
