window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let _x1a0f3n9_dsh_session_types = require("@x1a0f3n9/dsh-session/types");
...
`;
		...
		*/
		async function runRewindAndFill(session, seq, mode, currentSessionId, chatOf, _watchChat, setComposerText) {
			...
			try {
				...
				const deleted = await session.deleteFrom((0, _x1a0f3n9_dsh_session_types.SessionSeq)(seq));
				if (!deleted.ok) {
		...
		*/
		function deletionSeq(seq) {
			return (0, _x1a0f3n9_dsh_session_types.SessionSeq)(Math.trunc(seq));
		}
		/**
