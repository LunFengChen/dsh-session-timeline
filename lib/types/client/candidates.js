 ...
 */
import { hiddenSeqsOf } from "./hidden.js";
 ...
 */
export function messagePreviewOf(message) {
 ...
 */
export function formatCandidateTime(time) {
 ...
 */
export function rewindCandidatesOf(snap, hidden, limit = DEFAULT_CANDIDATE_LIMIT) {
    ...
    for (let i = snap.order.length - 1; i >= 0 && candidates.length < limit; i--) {
        ...
        if (node === undefined || (node.kind !== 'user' && node.kind !== 'steering'))
            continue;
        if (hidden.has(node.anchorSeq ?? node.data.seq))
            continue;
        candidates.push({
            seq: node.data.seq,
            time: node.data.time,
 ...
 */
export function rewindCandidatesOfChat(snap) {
 ...
 */
export function rewindOptionsOf(snap, t) {
 ...
 */
export function candidateBySeq(snap, seq) {
 ...
 */
export function rewindCandidatesFromHostText(text) {
    if (!text.startsWith(CANDIDATE_LIST_HEADER))
        return [];
    ...
    for (const line of lines) {
        ...
        if (!Number.isSafeInteger(seq) || !Number.isFinite(time))
            continue;
 ...
 */
export function rewindOptionsFromCandidates(candidates, t) {
 ...
 */
export function rewindOptionsFromHostText(text, t) {
