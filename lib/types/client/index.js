...
import { rewindCandidatesFromHostText, rewindCandidatesOfChat, rewindOptionsFromCandidates, } from "./candidates.js";
import { openPopover, knownCommandSeqs, waitForCommand } from "./popover.js";
import { createRewindBridge, runRewindAndFill } from "./portals.js";
import { TimelineActions } from "./actions.js";
import { CompactButton } from "./compact-button.js";
import { chatSnapshotOf, isCandidateCommand } from "./hidden.js";
import { rewindLog } from "./log.js";
import { BUILD_HASH, PLUGIN_VERSION } from "./build-info.js";
import { en, zh } from "./locales.js";
import { STYLE } from "./styles.js";
import { SettingsCleanupCard, CLEANUP_SETTINGS_NAMESPACE, } from "./settings-card.js";
...
export function apply(ctx) {
    ...
    ctx.effect(function* () {
        ...
        yield slots.inject('conversation.chat.user-actions', () => slots.register({ name: 'conversation.chat.user-actions', id: 'dsh-session-timeline-user-actions', order: 1000 }, ({ seq, content }) => (createElement(TimelineActions, { kind: 'user', seq, content, session: actionSession(), t }))));
        yield slots.inject('conversation.chat.assistant-actions', () => slots.register({ name: 'conversation.chat.assistant-actions', id: 'dsh-session-timeline-assistant-actions', order: 1000 }, ({ seq }) => (createElement(TimelineActions, { kind: 'assistant', seq, session: actionSession(), t }))));
        yield slots.inject('conversation.input.right', () => slots.register({ name: 'conversation.input.right', id: 'dsh-session-timeline-compact', order: 100 }, () => createElement(CompactButton, { session: actionSession(), t })));
        // ---- snapshot-cleanup settings card (Settings > Plugins > Plugin config) ----
        const cleanupScope = ctx.settingsScope.bind({
        ...
        const fetchHostCandidates = async (face, chatOf) => {
            ...
            if (outcome === null || outcome.kind !== 'success' || outcome.text === undefined)
                return undefined;
        ...
        const hostCandidatesCache = new Map();
            ...
            ui: {
                ...
                options: async (session) => {
                    ...
                    if (candidates !== undefined)
                        hostCandidatesCache.set(session.sessionId, candidates);
 ...
 */
export { hiddenSeqsOf, targetSeqOfArgs } from "./hidden.js";
