 ...
 */
import { hasFileImpact } from "./hidden.js";
...
function parseImpactList(text) {
    ...
    for (const line of text.split('\n')) {
        if (line.startsWith('restore:'))
            restores.push(line.slice('restore:'.length));
        else if (line.startsWith('delete:'))
            deletes.push(line.slice('delete:'.length));
    }
 ...
 */
export function knownCommandSeqs(session, chatOf, match) {
    const known = new Set();
 ...
 */
export function waitForCommand(session, chatOf, match, timeoutMs = 8000, watch) {
...
function openRetractPopover(opts) {
    ...
    if (retract === undefined || onRetract === undefined)
        return;
...
export function openPopover(opts) {
    ...
    if (seq === undefined || time === undefined || onRewind === undefined)
        return;
