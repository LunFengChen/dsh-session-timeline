...
export const DEFAULT_CLEANUP_CONFIG = { enabled: false, maxAgeDays: DEFAULT_MAX_AGE_DAYS };
/**
 ...
 */
export function settingsCleanupStore(scope) {
    return {
        load: () => scope.get(),
        save: async (next) => {
 ...
 */
export async function migrateLegacyCleanupConfig(legacyPath, scope, log) {
    ...
    if (!loaded.fromFile)
        return false;
    if (loaded.config.enabled === DEFAULT_CLEANUP_CONFIG.enabled
 ...
 */
export function resolveCleanupConfigPath(dshHome) {
 ...
 */
export function resolveCleanupStatePath(dshHome) {
 ...
 */
export async function loadLastSweepAt(path) {
    try {
 ...
 */
export async function saveLastSweepAt(path, ms) {
    const tmp = `${path}.tmp`;
 ...
 */
export async function runAutoCleanupCheck(deps, sessionId) {
    try {
        ...
        if (!loaded.config.enabled)
            return;
        if (!shouldRunAutoSweep(await loadLastSweepAt(deps.statePath), Date.now()))
            return;
 ...
 */
export function parseCleanupConfig(raw) {
 ...
 */
export async function loadCleanupConfig(path) {
    let text;
    try {
 ...
 */
export async function saveCleanupConfig(path, config) {
 ...
 */
export function parseCleanupCommand(rawInput) {
    ...
    switch (parts[0]) {
        ...
        case 'max-age': {
            if (parts.length !== 2)
                return { error: 'usage: /snapshot-auto-cleanup max-age <days>' };
            const days = Number(parts[1]);
            if (!Number.isInteger(days) || days <= 0)
                return { error: '"max-age" must be a positive integer (days)' };
 ...
 */
export function shouldRunAutoSweep(lastAtMs, nowMs) {
    return nowMs - lastAtMs >= AUTO_SWEEP_INTERVAL_MS;
}
//# sourceMappingURL=snapshot-cleanup.js.map