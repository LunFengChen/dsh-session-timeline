 ...
 */
export function isLinkEntry(entry) {
    return 'ref' in entry;
}
...
const RESTORE_JOURNAL_STATES = new Set(['running', 'rollback-running', 'completed', 'rolled-back', 'recovery-required']);
/**
...
function isRestoreJournal(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = value;
    if (typeof v.id !== 'string' || typeof v.sessionId !== 'string' || typeof v.targetSeq !== 'number')
        return false;
    if (typeof v.state !== 'string' || !RESTORE_JOURNAL_STATES.has(v.state))
        return false;
...
export class SnapshotStore {
    ...
    async ensureDedupSeeded(sessionId) {
        if (!this.dedup || this.seededSessions.has(sessionId))
            return;
     ...
     * path from a restore).
     */
    async resolveBefore(sessionId, entry, seen = new Set()) {
        if (!isLinkEntry(entry))
            return entry.before;
        const key = `${entry.anchorSeq}:${entry.callId}`;
        if (seen.has(key))
            throw new Error(`link cycle at ${entry.path} (${key})`);
        seen.add(key);
        if (!isSafeLinkRef(entry.ref))
            throw new Error(`unsafe link ref ${entry.ref} for ${entry.path}`);
        const referenced = await readEntry(join(this.sessionDir(sessionId), entry.ref));
        if (referenced === undefined)
            throw new Error(`dangling link ${entry.ref} for ${entry.path}`);
    ...
    async entriesAfter(sessionId, targetSeq) {
        ...
        for (const name of names) {
            ...
            if (!Number.isSafeInteger(anchorSeq) || anchorSeq < targetSeq)
                continue;
    ...
    async restoreAfter(sessionId, targetSeq, deleteFile, probe = defaultProbe, opts) {
        ...
        for (let i = 0; i < actions.length; i++) {
            ...
            if (action === undefined || journalAction === undefined)
                continue;
    ...
    async beginRestore(sessionId, targetSeq, actions, probe) {
        ...
        for (const action of actions) {
            ...
            if (rescueError !== undefined)
                journalAction.rescueError = rescueError;
    ...
    async readJournal(sessionId, opId) {
        ...
        if (!isRestoreJournal(parsed))
            throw new Error(`restore journal ${file} failed schema validation`);
        return parsed;
    }
    /**
    ...
    async listJournals(sessionId) {
        ...
        const corrupt = [];
        for (const name of names) {
            if (!name.startsWith(SnapshotStore.JOURNAL_PREFIX) || !name.endsWith('.json'))
                continue;
            try {
    ...
    async applyActionToDisk(kind, path, content, deleteFile) {
        ...
        if (content === null)
            throw new Error(`restore content missing for ${path}`);
    ...
    async continueRestore(sessionId, opId, deleteFile, probe = defaultProbe, opts) {
        ...
        if (journal === undefined)
            throw new Error(`restore journal ${opId} not found for session ${sessionId}`);
    ...
    async prune(sessionId, keep = MAX_ANCHOR_GROUPS, opts) {
        ...
        for (const seq of seqs.slice(excess)) {
            ...
            for (const file of files) {
                ...
                if (entry === undefined || !isLinkEntry(entry))
                    continue;
                if (!isSafeLinkRef(entry.ref))
                    continue; // unsafe/corrupt ref: never follow it
                const slash = entry.ref.indexOf('/');
                const refAnchor = slash === -1 ? Number.NaN : Number(entry.ref.slice(0, slash));
                if (!Number.isSafeInteger(refAnchor) || !doomed.has(refAnchor))
                    continue;
                let before;
                try {
                    before = await this.resolveBefore(sessionId, entry);
    ...
    async sessionStats(sessionId) {
        ...
        try {
            ...
            if (error.code === 'ENOENT')
                return { anchorGroups: 0, entries: 0, journals: 0, bytes: 0 };
            throw error;
        }
        let anchorGroups = 0;
        let entries = 0;
        ...
        for (const name of names) {
            ...
            if (st.isDirectory()) {
                if (!Number.isSafeInteger(Number(name)))
                    continue;
                anchorGroups++;
                let files;
                try {
 ...
 */
export async function reconcileTracked(store, sessionId, anchorSeq, tracked, probe = defaultProbe) {
    let recorded = 0;
    for (const path of tracked) {
        try {
            if (await probe.isLink(path))
                continue;
