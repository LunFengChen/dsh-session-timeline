...
import { eventsOf } from "./session-events.js";
import { readSettingsSection } from "./settings-locale.js";
import { formatCandidateList, listRewindCandidates, parseRewindTarget, planRewind, RewindError } from "./rewind.js";
import { execSessionCwd } from "./session-cwd.js";
import { reconcileTracked, SnapshotStore } from "./snapshot.js";
import { CLEANUP_SETTINGS_NAMESPACE, CleanupConfigSchema, DEFAULT_CLEANUP_CONFIG, migrateLegacyCleanupConfig, parseCleanupCommand, resolveCleanupConfigPath, resolveCleanupStatePath, runAutoCleanupCheck, saveLastSweepAt, settingsCleanupStore, } from "./snapshot-cleanup.js";
...
async function executeRewind(ctx, store, fs, invocation, rawTarget, mode, inflight, persistLog = true) {
    ...
    try {
        ...
        if (mode === 'both') {
            ...
            let outcome;
            try {
                outcome = await withDeadline(store.restoreAfter(agent.session.id, plan.targetSeq, path => unlink(path)), 30_000, 'rewind file restore timed out');
            }
            catch (error) {
                return {
                    kind: 'error',
    ...
    switch (parsed.action) {
        ...
        case 'max-age': {
            ...
            if (value === undefined)
                return { kind: 'error', text: t('cleanup.usage') };
            const loaded = await readCleanupPolicy();
            const next = { ...(loaded.ok ? loaded.config : DEFAULT_CLEANUP_CONFIG), maxAgeDays: value };
            try {
                await writeCleanupPolicy(next);
            }
            catch (error) {
